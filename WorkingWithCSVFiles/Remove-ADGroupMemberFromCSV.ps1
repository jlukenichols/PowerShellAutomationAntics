<#
.SYNOPSIS
    This script is meant to serve as an example of how to take a CSV file containing a map of usernames and group names and then remove each user from each group.
.DESCRIPTION
    This script is meant to serve as an example of how to take a CSV file containing a map of usernames and group names and then remove each user from each group.
.NOTES
    Author: Luke Nichols
...
:loopThroughCsvFile foreach ($line in $csvFileObject) {
    ...
    if ($testMode -eq $false) {
        Remove-ADGroupMember -Identity $($line.Group) -Members $($line.User)
    } else {
        ...
        Remove-ADGroupMember -Identity $($line.Group) -Members $($line.User) -WhatIf
    }
    ...
    Write-Host "Removing user $($line.User) from group $($line.Group)"
